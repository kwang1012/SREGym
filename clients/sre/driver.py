...
async def main():
    ...
    parser.add_argument(
        "--agent",
        type=str,
        default=os.getenv("AGENT_TYPE", "playbook"),
        choices=["playbook", "hypo", "react", "inter"],
        help="Agent type to use (default: playbook)",
    )
    parser.add_argument(
        "--logs-dir",
        type=str,
    ...
    logger.info(f"Agent type: {args.agent} ({agent_cls.__name__})")
    agent = agent_cls(
        logs_dir=logs_dir,
