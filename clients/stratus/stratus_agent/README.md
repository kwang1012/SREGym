...
Though fancy on text, all four stratus agents share mostly the same agent implementation. They are implemented as custom REACT agents, which all inherit from the `BaseAgent` class in [`base_agent.py`](https://github.com/xlab-uiuc/SREGym/blob/main/clients/stratus/stratus_agent/base_agent.py).
