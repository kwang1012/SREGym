...
There are two ways to setup SREArena for a remote cluster, please choose either approach (A) or (B) to run SREArena.

## (A) Run SREArena inside the cluster
If you want to run your agent and SREArena **inside** the cluster, please upload the _whole SREArena codebase_ to the control node, and follow steps 1), 2), 3), 4).

### 1) Exchange SSH keys
**Do this on your own machine that is controlling the cluster.**
Your nodes need to be able to ssh into each other to setup the cluster. If your nodes have different keys then the device you're running SREArena on, or if you're unsure, proceed with the following steps to exchange ssh keys between the nodes.
...
## (B) Run SREArena outside of the cluster
If you want to run your agent and SREArena **outside** of the cluster (e.g., on your own workstation), please follow steps 3), 4) (1 and 2 are no longer needed).
