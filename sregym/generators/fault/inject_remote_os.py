...
class RemoteOSFaultInjector(FaultInjector):
    ...
    def get_host_info(self):
        ...
        with open(f"{BASE_DIR}/../scripts/ansible/inventory.yml") as f:
            ...
            if "ansible_ssh_private_key_file" in variables:
                self.key_filename = os.path.expanduser(variables["ansible_ssh_private_key_file"])
        ...
        print("No worker nodes found in the inventory file, your cluster is not applicable for this fault injector")
        return None
    ...
    def inject_script_on_host(self, host: str, user: str, script: str, script_name: str):
        ...
        try:
            ssh.connect(host, username=user, key_filename=self.key_filename)
    ...
    def clean_up_script_on_host(self, host: str, user: str, script_name: str):
        ...
        try:
            ...
            if pid:
                ...
                print("Waiting for kubelet to restart...")
                time.sleep(3)
